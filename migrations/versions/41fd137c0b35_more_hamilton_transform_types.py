...
def upgradeTransferTypes():
    update_sql = ("update sample_transfer_type "
              ...
              "prior_transfer_type_id = :ptti"
              "where id = :id")

    insert_sql = ("insert into sample_transfer_type "
                  "(id, name, sample_transfer_template_id, "
                  " source_plate_count, destination_plate_count, prior_transfer_type_id) "
                  "select :id, :name, :stti, :spc, :dpc, :ptti "
                  "where not exists "
                  "(select * from sample_transfer_type where id = :id)")

    sql_param_names = ("id", "name", "stti", "spc", "dpc", "ptti")
